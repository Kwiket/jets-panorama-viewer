...
PSVHUD.prototype.renderMarkers = function () {
  ...
  PSVUtils.forEach(allMarkers, function (marker) {
    var isVisible = marker.visible;

    if (isVisible && marker.isPoly()) {
      var visiblePositions = this._getPolyPositions(marker)
        .filter(function (pos) { return !Number.isNaN(pos.x) && !Number.isNaN(pos.y) });

      isVisible = visiblePositions.length > (marker.isPolygon() ? 2 : 1);

      if (isVisible) {
        marker.position2D = this._getPolyDimensions(marker, visiblePositions);

        var points = visiblePositions.map(function (pos) {
          return pos.x + ',' + pos.y;
        }).join(' ');
      ...
      if (isVisible) {
        ...
        var correctedRotation = 0;
        if (marker.basePolylineAnchor) {
          var basePolyline = allMarkers[marker.basePolylineAnchor];
          if (basePolyline) {
            var visiblePositions = this._getPolyPositions(basePolyline);
            if (visiblePositions && visiblePositions.length > 1) {
              var horizontal = { x1: 0, y1: 0, x2: 10, y2: 0 };
              var startPos = visiblePositions[0];
              var endPos = visiblePositions[1];
              var angle = PSVUtils._getAngleBetween2Lines(
                horizontal.x1, horizontal.y1,
                horizontal.x2, horizontal.y2,
                startPos.x, startPos.y,
                endPos.x, endPos.y
              )
              correctedRotation = angle + (marker.angle || 0);
            }
          }
        }
        if (marker.isSvg()) {
          marker.$el.setAttributeNS(null, 'transform',
            ...
            (!marker.lockRotation && correctedRotation ? ' rotate(' + rotation + ')' : '')
          );
        }
        else {
          marker.$el.style.transform = 'translate3D(' + position.x + 'px, ' + position.y + 'px, 0px)' +
            (scale !== 1 ? ' scale(' + scale + ', ' + scale + ')' : '') +
            (!marker.lockRotation && correctedRotation ? ' rotate(' + correctedRotation + 'deg)' : '')
            ;
        }
      }
    }
...
/**
 * @summary Computes the angle between two lines
 * @param {number} ax1 x of first line start
 * @param {number} ay1 y of first line start
 * @param {number} ax2 x of first line end
 * @param {number} ay2 y of first line end
 * @param {number} bx1 x of second line start
 * @param {number} by1 y of second line start
 * @param {number} bx2 x of second line end
 * @param {number} by2 y of second line end
 * @returns {number} degree
 */
PSVUtils._getAngleBetween2Lines = function (ax1, ay1, ax2, ay2, bx1, by1, bx2, by2) {
  var deltaAx = ax2 - ax1;
  var deltaAy = ay2 - ay1;
  var deltaBx = bx2 - bx1;
  var deltaBy = by2 - by1;
  var angle = Math.atan2(deltaAx * deltaBy - deltaAy * deltaBx, deltaAx * deltaBx + deltaAy * deltaBy);
